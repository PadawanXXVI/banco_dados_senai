/*
Aula 06
DISTINCT  e Funções de Agregação (COUNT, SUM, AVG, MIN e MAX).
AS
LIMIT
*/
...
-- Uso do DISTINCT 
SELECT DISTINCT departamento FROM tb_funcionario; -- retorna os dados sem repetí-los.
SELECT DISTINCT cargo FROM tb_funcionario; -- retorna as entradas do cargo

-- Uso do COUNT
SELECT COUNT(*) FROM tb_funcionario; --  retorna o total de entradas, no caso, 30 registros

SELECT COUNT(*) FROM tb_funcionario
WHERE cargo = 'Analista'; --  total de registros de pessoas no cargo de Analista

SELECT COUNT(*) FROM tb_funcionario
WHERE genero = 'feminino'; -- total pessoas do sexo feminino

SELECT COUNT(*) FROM tb_funcionario
WHERE tipo_contrato = 'estagiario'; -- total de pessoas com contrato de estagiário

-- Uso do MAX - para saber o valor máximo  (somente para dados numéricos)
SELECT MAX(salario) FROM tb_funcionario;

SELECT MAX(salario) FROM tb_funcionario
WHERE tipo_contrato = 'estagiario';

SELECT MAX(salario), nome FROM tb_funcionario; -- além de informar o maior valor, traz junto o nome

SELECT MAX(salario), nome FROM tb_funcionario
WHERE tipo_contrato = 'estagiario'; --  maior salário de estagiário com o nome do funcionário

-- Uso do MIN - para saber o valor mínimo 
SELECT MIN(salario), nome FROM tb_funcionario; --  retorma o primeiro registro na tabela, tanto o mínimo quanto o máximo.

SELECT MIN(salario), nome FROM tb_funcionario
WHERE tipo_contrato = 'estagiario';

-- Uso do LIMIT para restringir o total de registro de devem ser retornados no consulta
SELECT * from tb_funcionario
LIMIT 10;

-- Uso do SUM - soma valores de uma determinada coluna
SELECT SUM(salario) FROM tb_funcionario;

-- Uso do AVG - calcula a média  
SELECT AVG(salario) FROM tb_funcionario;

-- Uso do AS - renomeia o nome da coluna no retorno da consulta
SELECT AVG(salario) AS `Média de salário`FROM tb_funcionario; -- antes retornava a coluna AVG(salario)
/*
Obs.:
Se o nome for com espaços entre as palavras, colocar entre ``(aspas) 
Se quiser usar o padrão de nome_separado_por_undescore, não precisa usar as ``(crases)
*/
