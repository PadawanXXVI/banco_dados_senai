...
## Visualizar os dados de uma coluna específica

```SQL
SELECT departamento FROM tb_funcionario;
```

## Uso do DISTINCT: retorna apenas as entradas únicas da coluna, ou seja, sem repetição
...
```SQL 
SELECT COUNT(*) FROM tb_funcionario; --  nesse caso retorna o total de registros na tabela
```
...
WHERE cargo = 'Analista'; -- retorna o total de registro de pessoas no cargo de Analista
```

## Uso do MAX: retorna o primeiro valor máximo (se houver mais de uma entrada com o mesmo maior valor máximo, retorna o primmeiro que aparece na tabela)

```SQL 
SELECT MAX(salario) FROM tb_funcionario;
```

## Uso do MAX com uma entrada específica

```SQL 
SELECT MAX(salario) FROM tb_funcionario
WHERE tipo_contrato = 'estagiario'; -- retorna o maior salário das pessoas cujo contrato é de estágio
```

## Uso do MIN: retorna o primeiro valor mínimo (se houver mais de uma entrada com o mesmo valor mínimo, retonra o primeiro que aparece na tabela)

```SQL
SELECT MIN(salario) FROM tb_funcionario;
```

## Uso do LIMIT: restringe o total de de resgitros que devem ser retornados na consulta.

```SQL
SELECT * FROM tb_funcionario
LIMIT 10; --  apenas os 10 primeiros registros encontrados
```

## Uso do SUM: soma os valores das colunas
